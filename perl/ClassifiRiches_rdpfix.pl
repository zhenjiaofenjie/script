...
my $USAGE = "USAGE:\nperl PhyloRichness.pl phylo_rdp_fixrank.txt abundance.txt <optional>threshold\n\nthreshold is the confidence threshold comes from RDP results, default 80%\n";
...
foreach (<Phylofile>){
	chomp;
	$_=~ s/\"//g;
	if(/(?<UniqueID>[0-9a-zA-Z\#\.\_]+);\+?\-?;(?<Kingdom>[^;]+);(?<Kconfidence>\d+)\%;(?<Phylum>[^;]+);(?<Pconfidence>\d+)\%;(?<Class>[^;]+);(?<Cconfidence>\d+)\%;(?<Order>[^;]+);(?<Oconfidence>\d+)\%;(?<Family>[^;]+);(?<Fconfidence>\d+)\%;(?<Genus>[^;]+);(?<Gconfidence>\d+)\%/){
		my $kingdom=$+{Kingdom};
		my $phylum=$+{Phylum};
		my $class=$+{Class};
		my $order=$+{Order};
